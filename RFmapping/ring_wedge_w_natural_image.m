function trial_records = ring_wedge_w_natural_image(debug_on)
  ...
  expt_info.free_viewing_time = free_viewing_time;
  expt_info.break_every = break_every;
  ...
  specificImage = [1,2,3,4];
  numImage = length(specificImage);  
  fls = dir(image_dir);
  
  all_rects = ones(1,4)*NaN;
  # make textures
  for i = 1:numImage
    ...
    if i == 1
      all_rects(i,:) = [0 0 3*s2 3*s1];
      all_rects(i,:) = CenterRectOnPoint(all_rects(i,:), round(screenXpixels/2), round(screenYpixels/2));
    end
  ...
  expt_info.theImageLocations = theImageLocations;
  ...
  while is_running
    ...
    % here make a condition to display an image on every nth trial
    if tr_ind == break_every
      % code to draw a natural image
      trImage = randi(numImage);
      stimulus_rect = all_rects(1,:);
      Screen('DrawTexture', stimulus_window, imageTextures_stimulus{trImage}, [], stimulus_rect, 0);
      Screen('DrawTexture', eyeTrack_window, imageTextures_eyeTrack{trImage}, [], stimulus_rect, 0);
      ...
      free_viewing_clock = tic();
      while toc(free_viewing_clock) < free_viewing_time;
        [keyIsDown, secs, keyCode, deltaSecs] = KbCheck();                    
      if ~mouse_track
        Datapixx('RegWrRd');
        status = Datapixx('GetAdcStatus');
    ...
    % end condition (wedge or natural image)
